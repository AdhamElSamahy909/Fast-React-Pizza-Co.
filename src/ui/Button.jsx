...
function Button({ children, disabled, to, type, onClick }) {
  ...
  const styles = {
    ...
    secondary:
      'inline-block text-sm rounded-full border-2 border-stone-300 px-4 py-2.5 font-semibold uppercase tracking-wide text-stone-400 transition-colors duration-300 hover:bg-stone-300 hover:text-stone-800 focus:bg-stone-300 focus:outline-none focus:ring focus:ring-stone-200 focus:text-stone-800 focus:ring-offset-2 disabled:cursor-not-allowed sm:px-6 sm:py-3.5',
  };
  if (to)
    return (
  ...
  return (
    <button // For the padding to take place, we need to make the button inline-block
      // We can't leave the focus state none as this will create accessibilty issues. Instead,
      // we can use something that Tailwind created for us which is what is called ring
